...
<form action="" method="post" enctype="multipart/form-data">
    <div class="row">
        <div class="form-group col-sm-3">
            ...
            <select name="category_child_id" class="form-control" id="category_id">
                <?php foreach ($categories as $category):
                    $selected = '';
                    if (isset($_POST['category_parent_id']) && $category['id'] == $_POST['category_child_id']) {
                        $selected = 'selected';
                    }
                    ?>
                    <option style="font-weight: bold;" disabled="disabled" id="number" value="<?php echo $category['id'] ?>" <?php echo $selected; ?>>
                        <?php echo $category['name'] ?>
                    </option>
                    <?php foreach ($category_childs as $category_child):
                    $selected = '';
                    if (isset($_POST['category_child_id']) && $category_child['id'] == $_POST['category_child_id']) {
                        $selected = 'selected';
                    }
                    ?>
                    <?php if ($category['id'] == $category_child['category_id']): ?>
                    <option class="category_item" value="<?php echo $category_child['id'] ?>" <?php echo $selected; ?>>
    ...
    <div class="form-group">
        ...
        <input type="text" name="seo_description"
               value="<?php echo isset($_POST['seo_description']) ? $_POST['seo_description'] : '' ?>"
    ...
    <div class="form-group">
        ...
        <input type="text" name="seo_keywords"
               value="<?php echo isset($_POST['seo_keywords']) ? $_POST['seo_keywords'] : '' ?>"
