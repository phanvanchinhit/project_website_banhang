...
//    mặc đinh page hiện tại là 1
        ...
        //lấy tổng số bản ghi dựa theo các điều kiện có được từ mảng params truyền vào
        $count_total = $category_model->countTotalChild();
        $params['total'] = $count_total;
        ...
        //lấy danh sách category sử dụng phân trang
        $category_child = $category_model->getAllPaginationChild($params);

        $this->content = $this->render('views/category_child/index.php', [
            //truyền biến $categories ra vew
    ...
    public function createChild()
    {
        if (isset($_POST['submit'])) {
            ...
            if (empty($this->error)) {
                ...
                $category_model->status = $status;
                //gọi phương thức insert
                $is_insert = $category_model->insertChild();
                if ($is_insert) {
                    $_SESSION['success'] = 'Thêm mới thành công';
                } else {
                    $_SESSION['error'] = 'Thêm mới thất bại';
                }
                header('Location: index.php?controller=category&action=indexChild');
                exit();
            }

        }
        $category_model = new Category();
        $category_child = $category_model->getAllCategory();
        //lấy nội dung view create.php
        $this->content = $this->render('views/category_child/create.php',[
            'category_child' => $category_child,
        ]);
        //gọi layout để nhúng nội dung view create vừa lấy đc
        require_once 'views/layouts/main.php';
    ...
    public function updateChild()
    {
        ...
        $id = $_GET['id'];
        $category_model = new Category();
        $category_childs = $category_model->getCategoryByIdChild($id);
        //submit form
        if (isset($_POST['submit'])) {
            $category_id = $_POST['category_id'];
            $name = $_POST['name'];
            ...
            if (empty($this->error)) {
                ...
                $category_model->updated_at = date('Y-m-d H:i:s');
                //gọi phương thức update theo id
                $is_update = $category_model->updateChild($id);
                if ($is_update) {
                    $_SESSION['success'] = 'Update thành công';
                } else {
                    $_SESSION['error'] = 'Update thất bại';
        ...
        //lấy nội dung view create.php
        $this->content = $this->render('views/category_child/update.php', [
    ...
    public function deleteChild()
    {
        ...
        $id = $_GET['id'];
        $category_model = new Category();
        $is_delete = $category_model->deleteChild($id);
        if ($is_delete) {
            $_SESSION['success'] = 'Xóa thành công';
        } else {
            $_SESSION['error'] = 'Xóa thất bại';
    ...
    public function detailChild()
    {
        ...
        $id = $_GET['id'];
        $category_model = new Category();
        $category = $category_model->getCategoryByIdChild($id);
        //lấy nội dung view create.php
        $this->content = $this->render('views/category_child/detail.php', [
            'category' => $category
        ]);
        //gọi layout để nhúng nội dung view detail vừa lấy đc
        require_once 'views/layouts/main.php';
...
//    mặc đinh page hiện tại là 1
        ...
        //lấy tổng số bản ghi dựa theo các điều kiện có được từ mảng params truyền vào
        $count_total = $category_model->countTotalCategoryNew();
        $params['total'] = $count_total;
        ...
        //lấy danh sách category sử dụng phân trang
        $categories = $category_model->getAllPaginationCategoryNew($params);

        $this->content = $this->render('views/category_news/index.php', [
            //truyền biến $categories ra vew
            'categories' => $categories,
            //truyền biến phân trang ra view
            'pages' => $pages,
    ...
    public function createNew()
    {
        if (isset($_POST['submit'])) {
            ...
            if (empty($this->error)) {
                ...
                if ($avatar_files['error'] == 0) {
                    ...
                    $avatar = time() . '-category_news-' . $avatar_files['name'];
                    move_uploaded_file($avatar_files['tmp_name'], $dir_uploads . '/' . $avatar);
                }
                //lưu vào csdl
                //gọi model để thực  hiện lưu
                ...
                $category_model->status = $status;
                //gọi phương thức insert
                $is_insert = $category_model->insertNew();
                if ($is_insert) {
                    $_SESSION['success'] = 'Thêm mới thành công';
                } else {
                    $_SESSION['error'] = 'Thêm mới thất bại';
                }
                header('Location: index.php?controller=category&action=indexNew');
                exit();
            }
        ...
        //lấy nội dung view create.php
        $this->content = $this->render('views/category_news/create.php');
        //gọi layout để nhúng nội dung view create vừa lấy đc
        require_once 'views/layouts/main.php';
    }
    ...
    public function updateNew()
    {
        ...
        $id = $_GET['id'];
        $category_model = new Category();
        $category = $category_model->getCategoryNewById($id);
        //submit form
        if (isset($_POST['submit'])) {
            $name = $_POST['name'];
            $description = $_POST['description'];
            ...
            if (empty($this->error)) {
                ...
                $category_model->updated_at = date('Y-m-d H:i:s');
                //gọi phương thức update theo id
                $is_update = $category_model->updateNew($id);
                if ($is_update) {
                    $_SESSION['success'] = 'Update thành công';
                } else {
                    $_SESSION['error'] = 'Update thất bại';
        ...
        //lấy nội dung view create.php
        $this->content = $this->render('views/category_news/update.php', ['category' => $category]);
        //gọi layout để nhúng nội dung view create vừa lấy đc
        require_once 'views/layouts/main.php';
    }
    ...
    public function deleteNew()
    {
        ...
        $id = $_GET['id'];
        $category_model = new Category();
        $is_delete = $category_model->deleteNew($id);
        if ($is_delete) {
            $_SESSION['success'] = 'Xóa thành công';
        } else {
            $_SESSION['error'] = 'Xóa thất bại';
    ...
    public function detailNew()
    {
        ...
        $category = $category_model->getCategoryNewById($id);
        //lấy nội dung view create.php
        $this->content = $this->render('views/category_news/detail.php', [
            'category' => $category
        ]);
        //gọi layout để nhúng nội dung view detail vừa lấy đc
        require_once 'views/layouts/main.php';
