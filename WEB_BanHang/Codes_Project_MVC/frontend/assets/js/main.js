...
$(document).ready( function () {
    $('#favLike').click(function(){
        $('a').load('index.php', function() {
...
// Xử lý thích san phẩm
$(document).ready(function () {
    $('.add-to-favs').click(function () {
        var product_id = $(this).attr('data-fav');
        //Gọi ajax sử dụng jQuery
        $.ajax({
            url: 'index.php?controller=favourite&action=add',
            method: 'GET',
            data: {
                product_id: product_id
            },
            success: function(data) {
                console.log(data);
                $('.ajax-message-favs')
                    .html('Bạn đã thích sản phẩm')
                    .addClass('ajax-message-favs-active');
                setTimeout(function(){
                    $('.ajax-message-favs').removeClass('ajax-message-favs-active')
                }, 4000);
                var favs_total = $('.favs-amount').html();
                favs_total++;
                $('.favs-amount').html(favs_total);
            }
        });
    });
});
